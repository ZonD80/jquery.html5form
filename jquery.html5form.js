...
(function ($) {
    $.fn.html5form = function (options) {
        $(this).each(function () {
            ...
            $.each($(':input:not(:button, :submit)', form), function (i) {
                fillInput($(this));
            ...
            $.each($('input:submit, input:image', this), function () {
                $(this).bind('click', function (ev) {
                    var emptyInput = null;
                    var emailError = null;
                    var input = $(':input:not(:button, :submit)', form);
                    $(required).each(function (key, value) {
                        if (value == undefined) {
                            return true
                        }
						//console.log($('input[name="' + $(this).attr('name') + '"]:checked'));
                        if (($(this).val() == $(this).attr('placeholder')) || ($(this).val() == '') || ($(this).attr('type') == 'checkbox' && !$(this).is(':checked')) || ($(this).attr('type') == 'radio' && $('input[name="' + $(this).attr('name') + '"]:checked').length==0)) {
                            emptyInput = $(this);
                    ...
                    $(email).each(function (key, value) {
                        ...
                        if ($(this).val().search(/[\w-\.]{1,}@([\w-]{2,}\.)*([\w-]{2,}\.)[\w-]{2,4}/i)) {
                            emailError = $(this);
                            return false
                        }
                        return emailError
                    ...
                    if (!emptyInput && !emailError) {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    $.each(form[0], function () {
                                        fillInput($(this).not(':hidden, :button, :submit'));
                                        $('select', form).each(function () {
                                            $(this).css('color', opts.colorOff);
                                            $(this).children('option:eq(0)').attr('selected', 'selected')
                                        });
                                        $(':radio', form).removeAttr('checked')
                                    })
                                }
                            })
                        } else {
                        ...
                        if (emptyInput) {
                            ...
                            if ($(emptyInput).attr('type') != 'checkbox' && $(emptyInput).attr('type') != 'radio') $(emptyInput).effect("highlight", {
                                color: 'red'
                            }, 300);
                            else if ($(emptyInput).attr('type') == 'checkbox') $(emptyInput).parents('label').effect("highlight", {
                                color: 'red'
                            }, 300);
							else if ($(emptyInput).attr('type') == 'radio') $('input[name="'+$(emptyInput).attr('name')+'"]').parents('label').effect("highlight", {
                                color: 'red'
                            }, 300);
                        } else if (emailError) {
                            ...
                            $(emailError).select().effect("highlight", {
                                color: 'red'
                            }, 300)
                        } else {
                            alert('Unknown Error')
